...
function Otherprofilelink({ edit, data, token, HandleEdit }) {
  const dept = useLocation().pathname.split("/")[2];

  // PERSONAL LINKS ARRAY
  const [links, setLinks] = useState(() => {
    // Safely initialize based on existing data
    if (data?.personal_link && typeof data.personal_link === 'object') {
      if (data.personal_link["Personal Link"] && Array.isArray(data.personal_link["Personal Link"])) {
        return data.personal_link["Personal Link"];
      }
      if (Array.isArray(data.personal_link)) {
        return data.personal_link;
      }
    }
    return [{ title: "", link: "" }];
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare the inner data structure
      const personalLinkData = {
        "Personal Link": cleanedLinks,
        "Google Scholar Link": googlelink
      };

      // WRAP IT IN THE personal_link OBJECT (THIS IS THE FIX)
      const payload = {
