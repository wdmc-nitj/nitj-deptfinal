...
function BaseTable({
  ...
  // Check if field should be a date picker
  ...
  // Convert to DD-MM-YYYY format for saving
  const convertToDDMMYYYY = (dateValue) => {
    if (!dateValue) return '';
  ...
  const sortedData = [...data]?.sort((a, b) => {
    ...
    // Special handling for numbers (like Year, Amount)
    ...
    else if (typeof aValue === 'string') aValue = aValue.toLowerCase();
  ...
  useEffect(() => {
    ...
    if (Editfeild >= 0 && data[Editfeild]) {
      const formattedData = { ...data[Editfeild] };
      // Format date fields for input (convert to YYYY-MM-DD) without changing original
      feild.forEach(field => {
        if (isDateField(field) && formattedData[field]) {
          formattedData[field] = formatDateForInput(formattedData[field]);
        }
      });
      setChangedata(formattedData);
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    // Format date fields to DD-MM-YYYY before saving
    const formattedData = { ...changedata };
    feild.forEach(field => {
      if (isDateField(field) && formattedData[field]) {
        formattedData[field] = convertToDDMMYYYY(formattedData[field]);
      }
    });
    ...
    if (Editfeild < 0) {
      ...
      updatedData[Editfeild] = formattedData;
    }
